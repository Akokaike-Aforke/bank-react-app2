...
export const AppProvider = ({ children }) => {
  ...
  const initialSelectedAccount =
    parseInt(localStorage.getItem("selectedAccount")) || 0;
  ...
  useEffect(() => {
    if (userId) {
      const userValue =
        parseInt(localStorage.getItem(`user_${userId}_key`)) || 0;
  ...
  const getFormattedDate = (dateTime) => {
    ...
    if (oldDate.getTime() - currentDate.getTime() === 0) {
    ...
    if (oldDate.getTime() - currentDate.getTime() === 1) {
